...
test("only operations write; director reads; superadmin and doctors do not inherit", async () => {
  const data = await fixture();
  ...
  for (const role of ["superadmin", "doctor", "finance"]) {
    ...
    await assert.rejects(service(context(uid), {action: "doctorAdminList", country: "CL", ids: [data.id]}), code("permission-denied"));
  }
