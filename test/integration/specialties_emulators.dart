...
import 'package:segunda_opinion_panel/repositories/firebase_specialty_repository.dart';
...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final repository = FirebaseSpecialtyRepository(
        database: () => database,
        auth: () => auth,
        initialize: () async {},
      );
      final suffix = DateTime.now().microsecondsSinceEpoch;
      const password = 'Only-emulator-Specialty-482!';
      ...
      Matcher failure(SpecialtyIssue issue) => isA<SpecialtyFailure>().having(
        (error) => error.issue,
        'issue',
        issue,
      );
      ...
      final input = SpecialtyInput(code, 'Especialidad de integración', '');
      await repository.save('CL', input);
      await expectLater(
        repository.save('CL', input),
        throwsA(failure(SpecialtyIssue.duplicate)),
      );
      ...
      await repository.save(
        'CL',
        SpecialtyInput(code, 'Nombre corregido', 'Descripción ficticia'),
        previous: first,
      );
      await expectLater(
        repository.save('CL', input, previous: first),
        throwsA(failure(SpecialtyIssue.conflict)),
      );
      ...
      await expectLater(
        ...
        throwsA(failure(SpecialtyIssue.denied)),
      );
