  ...
  return async function classify(actor, input) {
    ...
    return database.runTransaction(async transaction => {
      ...
      if (input.action === "intakeClassificationGet") {
        ...
        return view(input.id, previous, canonical.data().memberships[input.country].some(role => ["operations", "superadmin"].includes(role)) && request.status === "received" && (!assignment.exists || assignment.data().status === "released"), specialty);
      }
