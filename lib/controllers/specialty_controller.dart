...
class SpecialtyController extends ChangeNotifier {
  SpecialtyController(this.repository, this.principal, this.country);
  ...
  bool get allowed =>
      country == 'CL' &&
      PanelAccess.allows(principal, country, PanelModule.specialties);
  bool get canManage =>
  ...
  Future<bool> save(SpecialtyInput input, {Specialty? previous}) =>
      _perform(() async {
  ...
  Future<bool> setActive(Specialty previous, bool active) => _perform(() async {
  ...
  Future<bool> _perform(
    ...
    try {
      ...
      if (!_disposed) {
        issue = error is SpecialtyFailure
            ? error.issue
            : SpecialtyIssue.unavailable;
        if (clearOnError) {
          records = [];
          nextCursor = null;
        }
